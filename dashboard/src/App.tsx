    ...
    <div className="min-h-screen bg-white text-gray-900 dark:bg-gray-950 dark:text-white flex flex-col items-center py-12 px-4">
      <div className="w-full max-w-xl space-y-6">
        ...
        <div className="space-y-3">
          <input
            ...
            className="w-full bg-gray-100 border border-gray-300 dark:bg-gray-800 dark:border-gray-700 rounded-lg px-4 py-3 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition-colors"
          />
          <div>
            <input
              type="password"
        ...
        <div className="text-center text-sm text-gray-600 dark:text-gray-400 space-y-1">
          <p className="font-semibold text-gray-900 dark:text-white text-base">Things to Keep in Mind</p>
        ...
        {/* Form feedback */}
        {formError && (
          <div className="bg-red-50 border border-red-300 text-red-700 dark:bg-red-900/40 dark:border-red-700 dark:text-red-300 rounded-lg px-4 py-3 text-sm">
            {formError}
          </div>
        )}
        {previewResult && (
          <div className="bg-indigo-50 border border-indigo-300 dark:bg-indigo-900/40 dark:border-indigo-700 rounded-lg px-4 py-3 text-sm space-y-1">
            <p className="font-semibold">Preview Result</p>
            <p>
              <span className="font-mono text-indigo-700 dark:text-indigo-300">{previewResult.num_enqueued.toLocaleString()}</span>
              {' '}records would be deleted
            </p>
            {previewResult.message && !previewResult.dry_run && (
              <p className="text-gray-600 dark:text-gray-400">{previewResult.message}</p>
            )}
          </div>
        )}
        {submitResult && (
          <div className="bg-green-50 border border-green-300 dark:bg-green-900/40 dark:border-green-700 rounded-lg px-4 py-3 text-sm space-y-1">
            <p className="font-semibold">Job Submitted</p>
            <p>{submitResult.message}</p>
            {submitResult.job_id && (
              <p className="text-gray-600 dark:text-gray-400 font-mono text-xs break-all">ID: {submitResult.job_id}</p>
            )}
          </div>
        )}
        ...
        <div className="flex gap-3 justify-center">
          <button
            ...
            className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg font-semibold text-white transition-colors"
          >
          ...
          <button
            ...
            disabled={formDisabled}
            className="px-6 py-2.5 bg-red-600 hover:bg-red-500 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg font-semibold text-white transition-colors"
          >
        ...
        <div className="pt-6 border-t border-gray-200 dark:border-gray-800 space-y-4">
          ...
          <div className="flex flex-wrap justify-center items-center gap-2">
            <input
              ...
              className="flex-2 sm:flex-1 bg-gray-100 border border-gray-300 dark:bg-gray-800 dark:border-gray-700 rounded-lg px-4 py-2.5 placeholder-gray-400 dark:placeholder-gray-500 font-mono text-sm focus:outline-none focus:border-indigo-500 transition-colors w-full sm:w-auto"
            />
            <button
              onClick={handleCheckStatus}
              disabled={statusLoading || !jobId}
              className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg font-semibold text-sm text-white transition-colors whitespace-nowrap"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 bg-red-600 hover:bg-red-500 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg font-semibold text-sm text-white transition-colors whitespace-nowrap"
            >
          ...
          {jobStatus && (
            <div className="bg-gray-100 dark:bg-gray-800 rounded-lg px-4 py-4 text-sm space-y-2.5">
              <StatusRow label="State">
                <span className={STATE_COLORS[jobStatus.job_state] ?? 'text-red-600 dark:text-red-400'}>
                  {jobStatus.job_state}
                </span>
              </StatusRow>
      ...
      <footer className="mt-12 text-center text-xs text-gray-400 dark:text-gray-600">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="hover:text-gray-600 dark:hover:text-gray-400 underline transition-colors"
        >
          Jaz's project
        </a>
